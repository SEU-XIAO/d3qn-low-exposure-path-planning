...
from train.waypoint_curriculum import WaypointCurriculumConfig, WaypointManager
...
class PPOTrainer:
    def __init__(
        ...
        obs_channels = int(self.env._get_observation().shape[0])
        feature_dim = 256 if self.env_config.grid_size <= 15 else 512
        self.policy = ActorCriticCNN(in_channels=obs_channels, feature_dim=feature_dim).to(self.device)
        self.optimizer = torch.optim.Adam(self.policy.parameters(), lr=self.ppo_config.learning_rate)

        self.buffer = RolloutBuffer(
            self.ppo_config.rollout_steps,
            (obs_channels, self.env_config.grid_size, self.env_config.grid_size),
            self.device,
        )

        self.waypoint_manager = WaypointManager(
            WaypointCurriculumConfig(
                enabled=self.ppo_config.enable_waypoint_curriculum,
                waypoint_spacing=self.ppo_config.waypoint_spacing,
                waypoint_reach_radius=self.ppo_config.waypoint_reach_radius,
                waypoint_reward=self.ppo_config.waypoint_reward,
                replan_stuck_collisions=self.ppo_config.replan_stuck_collisions,
                w_len=self.ppo_config.stealth_w_len,
                w_vis=self.ppo_config.stealth_w_vis,
                w_slope=self.ppo_config.stealth_w_slope,
                w_turn=self.ppo_config.stealth_w_turn,
            )
        )
    ...
    def train(self, save_dir: str = "checkpoints") -> None:
        ...
        obs = self.env.reset()
        self.waypoint_manager.reset_for_episode(self.env)
        ...
        while self.global_step < cfg.total_steps:
            ...
            if progress >= cfg.progress_decay_start:
                ...
                self.env.current_progress_weight = self.env_config.progress_weight * (1.0 - frac)
            else:
            ...
            for _ in range(cfg.rollout_steps):
                ...
                reward, done, info = self.waypoint_manager.patch_step_result(self.env, reward, done, info)
                self.waypoint_manager.maybe_replan(self.env)
            ...
            for _ in range(cfg.ppo_epochs):
                for indices in self.buffer.sample(cfg.minibatch_size):
                    ...
                    new_log_probs, values, entropy = self.policy.evaluate(mb_obs, mb_actions, mb_masks)

                    ratio = torch.exp(new_log_probs - mb_old_log_probs)
                    surr1 = ratio * mb_advantages
                    surr2 = torch.clamp(ratio, 1.0 - cfg.clip_epsilon, 1.0 + cfg.clip_epsilon) * mb_advantages
    ...
    def _evaluate_and_save(self, save_path: Path) -> None:
        ...
        torch.save(self.policy.state_dict(), save_path / f"policy_{self.global_step}.pt")
